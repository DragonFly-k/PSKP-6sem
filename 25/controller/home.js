  ...
  switch (req.method) {
    ...
    case "POST":
      if (req.body.username && req.body.password) {
        try {
          const user = await prismaClient.users.findFirst({where: {username: req.body.username,password: req.body.password,},});
          ...
          res.cookie("id", user.id, {httpOnly: true, sameSite: "strict",});
...
exports.register = async (req, res) => {
  switch (req.method) {
    ...
    case "POST":
      if (req.body.username && req.body.password) {
        try {
          await prismaClient.users.create({ data: {username: req.body.username,password: req.body.password,email: req.body.email,role: "user",},});
          res.redirect("/login");
        } catch (error) {
          console.error(error);
