    ...
    case "GET":
      try {
        if (req.ability.can(admin.manage, admin.all)) {
          let users = await prismaClient.users.findMany({select: {id: true,username: true,email: true,role: true,}});
...
exports.infoByUserId = async (req, res) => {
  switch (req.method) {
    case "GET":
      try {
        if ( req.ability.can(access.read,subject(entity.users, { id: parseInt(req.params.id) }))
        ) {
          let user = await prismaClient.users.findUnique({where: {id: parseInt(req.params.id),},select: {id: true,username: true,email: true,role: true,},});
